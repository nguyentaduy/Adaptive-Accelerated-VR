...
def make_m_minibatches(n, m, minibatch_size):
    ''' Create m minibatches from the training set by sampling *with* replacement.
        This function may sample the training set multiple times.
    Parameters:
        n: the number of examples in the dataset
        m: number of minibatches to generate
    ...
    batches = []
    for i in range(m):
        batch_index = np.random.choice(n, minibatch_size, replace=True)
