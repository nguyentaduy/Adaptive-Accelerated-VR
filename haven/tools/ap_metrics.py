...
class APMonitor:
    ...
    def get_avg_score(self):
        results = compute_precision(self.gt_ann_list,
                                         self.pred_ann_list,
                                         iouType='segm',
                                         iouThr=0.5,
...
def evaluate_annList(pred_annList, gt_annList,
    ...
    if iouThrList is None:
        iouThrList = np.linspace(.5, 0.95, int(np.round((0.95 - .5) / .05)) + 1, endpoint=True)

    # print("iouThrList", iouThrList)
    recThrList = np.linspace(.0, 1.00, int(np.round((1.00 - .0) / .01)) + 1, endpoint=True)
...
def compute_AP_dict(ap, aind, mind, eval_dict, iouThr, iouThrList):
    ...
    if ap == 1:
        result_dict = {"AP%s" % (iouThr*100):np.array(per_class),
            "mAP%s"%(iouThr*100):mean_s}
    else:
        result_dict = {"RC%s" % (iouThr*100):np.array(per_class),
            "mRC%s"%(iouThr*100):mean_s}
