...
def SVRG_Loopless(score_list, closure, batch_size, D, labels, init_step_size, max_epoch=100,
    ...
    if r <= 0:
        ...
        p = 1/m
        if verbose:
            print('Info: set p=1/n by default')
    else:
        m = int(r * n)
        p = 1/m
        print('Info: set p = ', p)
    ...
    for k in range(max_epoch):
        ...
        # Create Minibatches:
        minibatches = make_minibatches(n, 10*m, batch_size) #make 10m minibatches to be safe.
        i=0
        while True:
            ...
            #break out with probability p
            u = np.random.uniform()
            if u < p:
                print("Breaking out at iteration: ", i)
                break
            i+=1
        t_end = time.time()
